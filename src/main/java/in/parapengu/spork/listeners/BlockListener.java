...
import in.parapengu.spork.event.map.BlockChangeEvent;
...
public class BlockListener implements Listener {
	...
	public void onBlockPlace(BlockPlaceEvent event) {
		...
		SporkMap map = null;
		try {
			Match match = Spork.getSlot().getMatch();
			map = (match != null ? match.getMap() : null);
		} catch(NullPointerException ignored) {
		}
		BlockChangeEvent change = new BlockChangeEvent(event, map, PlayerData.getPlayer(player), event.getBlockReplacedState(), event.getBlockPlaced().getState());
		Spork.get().getServer().getPluginManager().callEvent(change);
	}
	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		BlockChangeEvent change = new BlockChangeEvent(event, map, PlayerData.getPlayer(player), event.getBlock().getState(), newState);
		Spork.get().getServer().getPluginManager().callEvent(change);
	}
	...
	public void onBucketEmpty(PlayerBucketEmptyEvent event) {
		...
		BlockChangeEvent change = new BlockChangeEvent(event, map, PlayerData.getPlayer(player), oldState, newState);
		Spork.get().getServer().getPluginManager().callEvent(change);
	}
	...
	public void onEntityExplode(EntityExplodeEvent event) {
		...
		if(event.getEntity() instanceof TNTPrimed) {
			...
			if(tnt.getSource() instanceof Player) {
				player = PlayerData.getPlayer((Player) tnt.getSource());
			}
		}
