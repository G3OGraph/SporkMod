...
public class MapCommands {
	...
	public static void sendInfo(CommandSender sender, SporkLoader loader) {
	...
	public static void map(CommandContext cmd, CommandSender sender) throws CommandException {
		if(cmd.argsLength() == 0) {
			SporkLoader currentmap = Rotation.get().getMap().getLoader();
			sendInfo(sender, currentmap);
		} else {
			SporkLoader result = SporkFactory.getMap(cmd.getJoinedStrings(0));
			if(result == null)
	...
	public static void maps(CommandContext cmd, CommandSender sender) throws CommandException {
		...
		for(SporkLoader loader : SporkFactory.getMaps()) {
			rows.add(loader.getShortDescription());
		}
